document.addEventListener("DOMContentLoaded", ()=>{
    ...
    const bgImgObserver = new IntersectionObserver((e)=>{
        e.forEach((e)=>{
            if (e.isIntersecting) e.target.classList.add("show-bg-img");
            else e.target.classList.remove("show-bg-img");
        });
    });
    ...
    const newsObserver = new IntersectionObserver((e)=>{
        e.forEach((e)=>{
            if (e.isIntersecting) e.target.classList.add("show-h1");
            else e.target.classList.remove("show-h1");
        });
    });
    ...
    document.addEventListener("scroll", ()=>{
        ...
        if (scrollDistance >= 180) navBar.style.backgroundColor = "#000000";
    ...
    function updateCarousel(index) {
        items.forEach((item, i)=>{
            ...
            if (i === index) item.classList.add("carousel-active");
        });
        indicatorItems.forEach((item, i)=>{
            item.classList.remove("carousel-indicator-active");
            if (i == index) item.classList.add("carousel-indicator-active");
            console.log(item, i);
        });
        ...
        circles.forEach((circle, i)=>{
            ...
            if (i < information[index].difficulty) circle.classList.add("circle-active");
        });
    }
